...
public class PlayerManager : MonoBehaviour {
    ...
    public GameObject m_playerBoardPrefab;
    ...
    void CreatePlayerBoard()
    {
        ...
        GameObject playerBoard = Instantiate(m_playerBoardPrefab, 10f * Vector3.down, Quaternion.identity) as GameObject; // Create player board to hold all this player's objects
        m_playedArea = playerBoard.transform.InstantiateChild(m_cardHolderPrefab);
        ...
        m_playerCamera = playerBoard.transform.InstantiateChild(m_playerCameraPrefab, new Vector3(0f, 2f, -10f)).GetComponent<Camera>();
    }

    void CreatePlayerDeck()
    {
        m_cardsInDeck = GameManager.m_cardManager.CreateDeck(m_deck, m_playerCamera, CardManager.DeckType.player);

        for (int i = 0; i < m_cardsInDeck.Count; i++)
        {
            m_cardsInDeck[i].Init(Location.deck, Vector3.zero, m_playerCamera);
        }
    }
    ...
    //CARD TRACKING
    //**********
    private List<CardObject> m_cardsInDeck = new List<CardObject>();
    private List<CardObject> m_cardsInHand = new List<CardObject>();
    private List<CardObject> m_cardsInPlay = new List<CardObject>();
    private List<CardObject> m_cardsInDiscard = new List<CardObject>();
    ...
    public void MoveToPlayArea(CardObject card)
    {
        card.SetLocation(Location.play);
        card.SetTargetPos(m_playedArea.transform.position);
    }
    ...
    public void MoveToHand(CardObject card)
    {
        ...
        card.SetHomePos(m_hand.transform.position + Vector3.right * m_cardsInHand.Count * m_cardSlotWidth / 2f);
        ChangeCardLocation(card, Location.hand);
        m_cardsInHand.Add(card);
    }

    void ShiftCardHome(CardObject card, Vector3 target, float delta)
    {
        card.SetHomePos(Vector3.MoveTowards(card.GetHomePos(), target, delta));
    }

    void ChangeCardLocation(CardObject card, Location newLocation)
    {
